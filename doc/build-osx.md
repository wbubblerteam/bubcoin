...
This guide describes how to build bubcoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Bubcoin Core from source.
...
### 4. Clone Bubcoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Bubcoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `bubcoind` or  `bubcoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/bubcoin/bubcoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Bubcoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/bubcoin/bubcoin/issues/7714).
...
You can deploy a `.dmg` containing the Bubcoin Core application using `make deploy`.
...
Run the following in your terminal to compile Bubcoin Core:
...
## Running Bubcoin Core

Bubcoin Core should now be available at `./src/bubcoind`.
If you compiled support for the GUI, it should be available at `./src/qt/bubcoin-qt`.

The first time you run `bubcoind` or `bubcoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Bubcoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Bubcoin"

touch "/Users/${USER}/Library/Application Support/Bubcoin/bubcoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Bubcoin/bubcoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Bubcoin/debug.log
```

## Other commands:

```shell
./src/bubcoind -daemon      # Starts the bubcoin daemon.
./src/bubcoin-cli --help    # Outputs a list of command-line options.
./src/bubcoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/bubcoin-qt -server # Starts the bubcoin-qt server mode, allows bubcoin-cli control
```
