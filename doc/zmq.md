...
The Bubcoin Core daemon can be configured to act as a trusted "border
router", implementing the bubcoin wire protocol and relay, making
...
The ZeroMQ feature in Bubcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bubcoind:
...
For instance:
    ...
    $ bubcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/bubcoind.tx.raw \
...
These options can also be provided in bubcoin.conf.
...
From the perspective of bubcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bubcoind directly. Furthermore, no information is
...
using. Bubcoind appends an up-counting sequence number to each
