...
* Update translations see [translation_process.md](https://github.com/bubcoin/bubcoin/blob/master/doc/translation_process.md#synchronising-translations).
...
* On both the master branch and the new release branch:
  ...
  - update `CLIENT_VERSION_MAJOR`, `PACKAGE_VERSION`, and `PACKAGE_STRING` in [`build_msvc/bubcoin_config.h`](/build_msvc/bubcoin_config.h)
...
- Translations on Transifex
    - Create [a new resource](https://www.transifex.com/bubcoin/bubcoin/content/) named after the major version with the slug `[bubcoin.qt-translation-<RRR>x]`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/bubcoin_en.xlf` to create it.
    ...
    - Make an announcement that translators can start translating for the new version. You can use one of the [previous announcements](https://www.transifex.com/bubcoin/bubcoin/announcements/) as a template.
    - Change the auto-update URL for the resource to `master`, e.g. `https://raw.githubusercontent.com/bubcoin/bubcoin/master/src/qt/locale/bubcoin_en.xlf`. (Do this only after the previous steps, to prevent an auto-update from interfering.)
...
- Translations on Transifex
    - Change the auto-update URL for the new major version's resource away from `master` and to the branch, e.g. `https://raw.githubusercontent.com/bubcoin/bubcoin/<branch>/src/qt/locale/bubcoin_en.xlf`. Do not forget this or it will keep tracking the translations on master instead, drifting away from the specific major release.
...
To tag the version (or release candidate) in git, use the `make-tag.py` script from [bubcoin-maintainer-tools](https://github.com/bubcoin-core/bubcoin-maintainer-tools). From the root of the repository run:

    ../bubcoin-maintainer-tools/make-tag.py v(new version, e.g. 0.20.0)
...
Check out the source code in the following directory hierarchy.
    ...
    git clone https://github.com/bubcoin-core/guix.sigs.git
...
Generate the change log. As this is a huge amount of work to do manually, there is the `list-pulls` script to do a pre-sorting step based on github PR metadata. See the [documentation in the README.md](https://github.com/bubcoin-core/bubcoin-maintainer-tools/blob/master/README.md#list-pulls).
...
Checkout the Bubcoin Core version you'd like to build:
...
Add other builders keys to your gpg keyring, and/or refresh keys: See `../bubcoin/contrib/builder-keys/README.md`.
...
However if this is done, once the release has been tagged in the bubcoin-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds.

Codesigner only: Commit the detached codesign payloads:

```sh
pushd ./bubcoin-detached-sigs
...
- Upload to the bubcoincore.org server (`/var/www/bin/bubcoin-core-${VERSION}`):
    1. The contents of `./bubcoin/guix-build-${VERSION}/output`, except for
       `*-debug*` files.
       ...
       as save storage space *do not upload these to the bubcoincore.org server,
...
- Create a torrent of the `/var/www/bin/bubcoin-core-${VERSION}` directory such
  that at the top level there is only one file: the `bubcoin-core-${VERSION}`
  directory containing everything else. Name the torrent
  `bubcoin-${VERSION}.torrent` (note that there is no `-core-` in this name).
  ...
  people without access to `bubcoincore.org` to download the binary distribution.
...
- Update other repositories and websites for new version
  ...
  - bubcoincore.org RPC documentation update
      ...
      - Install the new Bubcoin Core release

      - Run bubcoind on regtest

      - Clone the [bubcoincore.org repository](https://github.com/bubcoin-core/bubcoincore.org)

      - Run: `go run generate.go` while being in `contrib/doc-gen` folder, and with bubcoin-cli in PATH
...
- Announce the release:
  ...
  - Bubcoin Core Twitter https://twitter.com/bubcoincoreorg
