# User-space, Statically Defined Tracing (USDT) for Bubcoin Core

Bubcoin Core includes statically defined tracepoints to allow for more
...
## Adding tracepoints to Bubcoin Core
...
Multiple tools can list the available tracepoints in a `bubcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Bubcoin Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/bubcoind
stap net        outbound_message 0x0000000000107c05 /src/bubcoind
stap validation block_connected  0x00000000002fb10c /src/bubcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Bubcoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bubcoind | grep NT_STAPSDT -A 4 -B 2
